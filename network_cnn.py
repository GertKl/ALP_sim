...
# -*- coding: utf-8 -*-
"""
Created on Fri May 10 12:11:00 2024
...
class Network(swyft.AdamWReduceLROnPlateau, swyft.SwyftModule):#, supernet.Network):
    ...
    def __init__(self, nbins, marginals, param_names, **kwargs):
        ...
        self.net_data = torch.nn.Sequential(
            torch.nn.Conv1d(in_channels=1, out_channels=channels, kernel_size=kernel_size),
            torch.nn.Flatten(),
            torch.nn.LazyLinear(self.logratio_features),
            torch.nn.ReLU(),
        )
    ...
    def head_net(self, A):
        data = self.norm_data(A['data']).unsqueeze(1)
